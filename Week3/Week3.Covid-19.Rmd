...
Part B is my work to further look into covid cases in Hong Kong.
...
```
```{r create a linear model of deaths_per_thou based on cases_per_thou of US State and plot result prediction against actuals}
...
```
### My exploration of Hong Kong Data
...
```
```{r calc new cases and death per day}
...
```{r}
HK_mod <- lm(deaths ~ cases, data=Hong_Kong_cases)

Hong_Kong_cases_w_pred <- Hong_Kong_cases %>% mutate(pred=predict(HK_mod))

Hong_Kong_cases_w_pred %>% ggplot() +
