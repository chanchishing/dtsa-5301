...
## Create a simple model for deaths per thousand based on case per thousand of Hong Kong
Calculate cases per thousand and deaths per thousand in Hong Kong and create a linear model of it then plot the model against actual data.
...
HK_mod2 <- lm(deaths_per_thou ~ cases_per_thou, data=Hong_Kong_cases)

Hong_Kong_cases_w_pred2 <- Hong_Kong_cases %>% mutate(pred=predict(HK_mod2))

Hong_Kong_cases_w_pred2 %>% ggplot() +
geom_point(aes(x=cases_per_thou,y=deaths_per_thou),color="blue")+
geom_point(aes(x=cases_per_thou,y=pred),color="red") +
labs(title="Linear model of deaths per thousand against cases per thousand of Hong Kong",y=NULL)
